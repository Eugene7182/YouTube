...
- `POST /scheduler/seed-month` — формирует 30-дневный план (3 слота в сутки) и сохраняет его в `data/schedule.json`.
- `POST /scheduler/tick` — выбирает готовые к публикации темы из расписания, рендерит шорт и при необходимости загружает на YouTube.
...
## Автопостинг 3/день на месяц (бесплатно)

1. На Render при необходимости задайте переменные окружения: `SHORTS_SIZE=720x1280`, `SHORTS_FPS=24`, `SHORTS_MAX_SECS=12` — они ограничивают разрешение, FPS и длительность шорта, чтобы не выходить за лимиты бесплатного тарифа.
2. В репозитории GitHub откройте **Settings → Secrets and variables → Actions → New repository secret** и создайте секрет `ADMIN_TOKEN` со значением вашего административного токена.
3. Workflow `.github/workflows/shorts-cron.yml` автоматически вызывается трижды в сутки (02:30, 08:30, 14:30 UTC) и дёргает `/scheduler/tick`, чтобы собрать и загрузить по одному ролику из очереди.
4. Первого числа месяца в 00:20 UTC тот же workflow запускает `/scheduler/seed-month` и создаёт 90 роликов (3 слота × 30 дней). План сохраняется в `data/schedule.json`.
5. Для ручного пересидирования используйте:
   ```bash
   curl -X POST https://<host>/scheduler/seed-month \
     -H "Authorization: Bearer <token>" \
     -H "Content-Type: application/json" \
     -d '{"start":"2025-10-01","slots":["09:00","15:00","21:00"],"topics":[]}'
   ```
6. Если нужно собрать ролик немедленно, вызовите `/scheduler/tick`:
   ```bash
   curl -X POST https://<host>/scheduler/tick \
     -H "Authorization: Bearer <token>" \
     -H "Content-Type: application/json" \
     -d '{"limit":1,"upload":true}'
   ```

GitHub Actions автоматически прогревают Render перед вызовом API, поэтому дополнительных Cron-задач на стороне Render не требуется.
